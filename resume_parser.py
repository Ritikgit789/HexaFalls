...
class ResumeParser:
    ...
    def extract_projects_from_resume(text, max_projects=3):
        """
        Extracts up to `max_projects` project entries with tech stacks and idea highlights.
        Looks for patterns like:
        - "Project: ..."
        - "Technology used: ..."
        - "Used X, Y, Z in this project"
        """

        # Pattern for common project start
        project_block_pattern = re.findall(
            r"(Project(?: Title)?\s*[:\-]?\s*.+?(?=(?:Project|Experience|Skills|$)))",
            text,
            re.IGNORECASE | re.DOTALL
        )

        project_details = []
        for block in project_block_pattern[:max_projects]:
            project = {}

            # Extract project title
            title_match = re.search(r"Project(?: Title)?\s*[:\-]?\s*(.*)", block, re.IGNORECASE)
            project["title"] = title_match.group(1).strip() if title_match else "Not Found"

            # Extract tech stack
            tech_match = re.findall(
                r"(?:Technology|Tech Stack|Tools|Languages)\s*(?:used)?\s*[:\-]?\s*(.*)",
                block, re.IGNORECASE)
            if tech_match:
                project["tech_stack"] = [t.strip() for t in re.split(r"[,\|;/]", tech_match[0])]
            else:
                # Try generic fallback
                tech_fallback = re.findall(r"\b(?:used|built using|implemented in)\b\s+([A-Za-z0-9, \-]+)", block, re.IGNORECASE)
                project["tech_stack"] = tech_fallback[0].split(",") if tech_fallback else []

            # Extract 1–2 lines of idea or algorithm used
            idea_match = re.search(r"(?:Description|Objective|Built to|Idea|Goal)\s*[:\-]?\s*(.*)", block, re.IGNORECASE)
            project["idea"] = idea_match.group(1).strip() if idea_match else "Not mentioned"
